...
class MetadataBBoxContractTest(unittest.TestCase):
    ...
    def test_metadata_report_promotes_only_exact_highlights(self) -> None:
        ...
        self.assertLessEqual(audit["exact_quote_match_count"], audit["span_sequence_candidate_count"])
        self.assertGreater(audit["exact_quote_match_count"], 0)
        self.assertLessEqual(audit["subspan_match_candidate_count"], audit["span_sequence_candidate_count"])
        self.assertGreaterEqual(audit["bbox_union_candidate_count"], 0)
        self.assertGreaterEqual(audit["bbox_union_not_supported_count"], 0)
        self.assertGreaterEqual(audit["new_exact_promotion_count"], 0)
        self.assertLessEqual(audit["new_exact_promotion_count"], audit["exact_quote_match_count"])
        self.assertGreaterEqual(audit["kept_non_exact_after_attempt_count"], 0)
    ...
    def test_promotion_decisions_cover_non_exact_rows(self) -> None:
        ...
        for row in non_relation_decisions:
            ...
            if row["decision"] == "recovered_exact":
                self.assertEqual(row["promotion_attempt_result"], "recovered_exact", row["decision_id"])
                ...
                self.assertEqual(row["promotion_attempt_result"], "blocked_after_actual_attempt", row["decision_id"])
