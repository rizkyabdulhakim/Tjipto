...
class ArtifactOriginContractTest(unittest.TestCase):
    def test_manifest_file_records_have_origin_metadata(self) -> None:
        ...
        self.assertEqual(len(manifest["files"]), len(UUD_ARTIFACT_ORIGIN_POLICY))
    ...
    def test_validation_report_records_artifact_origin_health(self) -> None:
        ...
        self.assertEqual(health["manifest_file_rows"], len(UUD_ARTIFACT_ORIGIN_POLICY))
        self.assertEqual(health["files_with_origin"], len(UUD_ARTIFACT_ORIGIN_POLICY))
