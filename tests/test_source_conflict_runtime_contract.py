...
class SourceConflictRuntimeContractTest(unittest.TestCase):
    ...
    def test_exact_source_conflict_provenance_can_resolve_existing_viewer_policy(self) -> None:
        ...
        self.assertEqual(result["trace_support"][0]["authority_kind"], "source_conflict_provenance")
        self.assertEqual(result["trace_support"][0]["support_kind"], "source_anomaly_provenance")
        self.assertEqual(result["trace_support"][0]["finality_policy"], "source_anomaly_provenance")
        self.assertEqual(result["trace_support"][0]["source_anomaly_kind"], "renumbering_provenance")
        self.assertEqual(result["trace_support"][0]["source_mapping_kind"], "historical_to_canonical_mapping")
        self.assertFalse(result["trace_support"][0]["citation_final"])
        self.assertEqual(result["trace_support"][0]["provenance_highlight_scope"], "all_relevant_spans")
        viewer = self.service.viewer("uud", result["trace_support"][0]["evidence_id"])
    ...
    def test_pasali_and_pasal_ii_source_routing_preserves_historical_provenance(self) -> None:
        ...
        self.assertFalse(pasal_i["citations"])
        self.assertTrue(pasal_i["historical_citations"])
        self.assertEqual(pasal_i["historical_citations"][0]["citation"], "Pasal I")
        self.assertFalse(pasal_i["historical_citations"][0]["citation_final"])
