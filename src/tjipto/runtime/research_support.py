"""Corpus-policy projections used by the legal research control plane."""
...
def research_semantic_terms(
    ...
    if isinstance(entity_aliases, dict):
        excluded.update(
            ...
            for value in (entity, *_string_tuple(entity_aliases.get(entity)))
            for token in meaningful_tokens(value, aliases=aliases)
        )
...
def research_semantic_term_groups(store: EvidenceStore, query: str) -> tuple[tuple[str, ...], ...]:
    ...
    related_tokens = {
        ...
        for token in (key, *_string_tuple(values))
    ...
    requested = {
        ...
        if not any(
            ...
            and any(
                term in meaningful_tokens(value, aliases=aliases)
                for value in _string_tuple(related.get(other))
            )
    ...
    for term in sorted(requested):
        ...
        for value in _string_tuple(key_values):
    ...
    groups.extend(
        group
        for alternatives_for_term in alternatives
        for group in alternatives_for_term
...
def semantic_support_context_terms(store: EvidenceStore, aliases: dict[str, str]) -> set[str]:
    ...
    return {
        token
        for phrase in policy.get("semantic_context_terms", ())
        if isinstance(phrase, str)
        for token in tokens(phrase, aliases=aliases)
    }
...
def semantic_supports_text(store: EvidenceStore, query: str, source: str) -> bool:
    ...
    analysis_signal = contains_intent_phrase(query, tuple(intent.get("instrument_analysis_signals", ()) or ()))
    research = store.config.setting("research", {}) or {}
    analysis = research.get("operation_requirements", {}).get("analyze", {}) if isinstance(research, dict) else {}
    issue_terms = tuple(analysis.get("issue_reference_terms", ()) or ()) if isinstance(analysis, dict) else ()
    # A bare interpretive request must not be answered by an adjacent lexical
    # definition. Issue-bearing legal analysis remains eligible for its
    # configured evidence requirements.
    if analysis_signal and issue_terms and not contains_intent_phrase(query, issue_terms):
        return False
    ...
    numeric_alias_values = tuple(
        str(value)
        for key, value in aliases.items()
        if key in numeric_words or re.search(rf"\b{re.escape(key)}\b", normalized_query)
    )
    ...
    framing_missing = {
        term for term in missing if corpus_frequency.get(term, 0) > max(10, len(store.evidence) // 20)
    }
...
def _semantic_support_score(
    ...
    analysis = (store.config.setting("research", {}) or {}).get("operation_requirements", {})
    analysis = analysis.get("analyze", {}) if isinstance(analysis, dict) else {}
    issue_terms = tuple(value for value in analysis.get("issue_reference_terms", ()) if isinstance(value, str))
    issue_match = any(contains_intent_phrase(normalized_query, (term,)) for term in issue_terms)
    issue_match = issue_match or any(
        contains_intent_phrase(normalized_query, (value,))
        for term in issue_terms
        for value in related.get(normalize_intent_text(term), ())
        if isinstance(value, str)
    )
    if issue_match:
        issue_tokens = {
            token for term in issue_terms for token in meaningful_tokens(term, aliases=aliases)
        }
        for term in requested:
            if term in issue_tokens:
                alternatives[term].update(issue_tokens)
    matched: set[str] = set()
    for term in requested:
        ...
        if len(term) < 6:
            continue
        if any(
            len(candidate) >= 6 and SequenceMatcher(None, term, candidate).ratio() >= 0.88
