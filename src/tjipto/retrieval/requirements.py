"""Deterministic retrieval-policy ownership for research evidence requirements.

This module turns the immutable query contract and corpus policy into bounded
research intent and typed requirements. It never retrieves or publishes evidence.
"""
...
from tjipto.retrieval.structured import has_structured_target, structured_occurrence_references
from tjipto.runtime.query_semantics import QuerySemantics
from tjipto.retrieval.research_support import (
    research_entities,
    research_entity_labels,
    research_focus_query,
    research_issue_term_groups,
...
def semantic_orchestration_required(store: EvidenceStore, query: str, semantics: QuerySemantics) -> bool:
    ...
    if semantics.requested_function != "retrieval" and semantics.operation != "analyze":
        return False
    # Exact structural targets are owned by structured retrieval.  Letting a
    # provider planner add semantic requirements here can turn a resolvable
    # heading into an unnecessary sufficiency failure.
    ...
    if not any(
        (semantics.requires_multiple_supports, semantics.requires_comparison, semantics.requires_decomposition, semantics.requires_graph)
    ):
...
def research_intent_for_ask(
    ...
    comparison = (len(entity_dimensions) > 1 or len(instrument_scopes) > 1) and (
        configured("comparison") or configured("authority") or configured("relation") or len(instrument_scopes) > 1
    )
...
def semantic_scope_covered(
    ...
    if semantics.source_role and semantics.source_role not in planned_roles and requirements:
        return False
    complex_signal = _hinted(hints, normalized, "comparison") or _hinted(hints, normalized, "procedure") or _hinted(hints, normalized, "relation")
    shared_entity_requirement = any(entity_labels <= set(requirement.required_entities) for requirement in requirements)
    if len(entity_labels) > 1 and (complex_signal or len(requirements) > 1) and len(requirements) < 2 and not shared_entity_requirement:
        return False
    if complex_signal and not requirements and not single_support_covers_query(store, query):
        return False
    return not (semantics.relation_intent and len(entity_labels) > 1 and not entity_labels <= planned_entities)
...
def research_requirements_for_ask(
    ...
    *,
    ...
    evidence_roles = string_tuple(broad_comparison.get("evidence_roles")) if isinstance(broad_comparison, dict) else ()
    comparison_roles = string_tuple(broad_comparison.get("source_roles")) if isinstance(broad_comparison, dict) else ()
...
def _source_occurrence_requirements(
    ...
    if structural_references:
        ...
        constrained = bool(legal_units_by_role)
        roles = tuple(legal_units_by_role) if semantics.legal_references and constrained else tuple(semantics.source_scopes)
...
def _source_occurrence_legal_units(
    ...
    if references:
        ...
        if exact:
            return exact
    groups = research_issue_term_groups(store, query) or research_semantic_term_groups(store, query)
    if not groups:
        return {}
    ...
    for role in getattr(store.config, "source_roles", ()):
        candidates = tuple(
            ...
            and any(set(group) <= meaningful_tokens(_requirement_row_text(row), aliases=aliases) for group in groups)
            and semantic_supports_all_terms(store, query, _requirement_row_text(row))
        )
        ids = _prefer_specific_unit_ids(store, candidates)
        if ids:
            result[str(role)] = ids
    return result
...
def _reference_units_by_role(store: EvidenceStore, references: tuple[str, ...]) -> dict[str, tuple[str, ...]]:
    requested = tuple(
        {_normalise_reference(part) for part in _reference_parts(store, value) if part}
        ...
        ids = tuple(
            dict.fromkeys(
                ...
                if (
                    ...
                    and any(parts <= _normalised_reference_labels(row) for parts in requested)
                )
            )
        )
        if ids:
...
def _normalised_reference_labels(row: dict) -> set[str]:
    ...
    return {
        label
        for value in (row.get("citation"), *hierarchy)
        if (label := _normalise_reference(value))
    }
...
def _historical_requirements(
    ...
    deletion_terms = tuple(str(value).casefold() for value in deletion_policy.get("terms", ()) if isinstance(value, str))
    return (
        EvidenceRequirement(
...
def _analysis_requirements(
    ...
    if not isinstance(analysis_policy, dict):
        analysis_policy = {}
    source_role = str(analysis_policy.get("source_role") or "") or getattr(store.config, "preferred_source_role", None)
    ...
    if not references:
        references = _analysis_references_from_evidence(
            store,
            query,
            source_role,
            research_issue_term_groups(store, query),
    ...
    requirements = [
        EvidenceRequirement(
            ...
            explicit_references=_reference_parts(store, reference),
    ...
    limitations = _analysis_references_from_evidence(
        store,
...
def _analysis_references_from_evidence(
    store: EvidenceStore,
    query: str,
    ...
    *,
    ...
    if not groups:
        terms = meaningful_tokens(query, aliases=aliases)
        frequencies = {term: sum(term in meaningful_tokens(_requirement_row_text(row), aliases=aliases) for row in rows) for term in terms}
        present = tuple((frequency, term) for term, frequency in frequencies.items() if frequency)
        groups = ((min(present)[1],),) if present else ()
    candidates = tuple(
        row for row in rows if any(set(group) <= meaningful_tokens(_requirement_row_text(row), aliases=aliases) for group in groups)
    )
    selected_ids = _prefer_specific_unit_ids(store, candidates) if prefer_specific else _prefer_parent_unit_ids(store, candidates)
    by_id = {str(row.get("legal_unit_id")): row for row in candidates}
    return tuple(dict.fromkeys(reference for unit_id in selected_ids if (reference := _reference_for_requirement(by_id[unit_id]))))


def _requirement_row_text(row: dict) -> str:
    return " ".join(str(value or "") for value in (row.get("citation"), row.get("hierarchy"), row.get("quoted_text")))


def _prefer_specific_unit_ids(store: EvidenceStore, rows: tuple[dict, ...]) -> tuple[str, ...]:
    ids = tuple(dict.fromkeys(str(row.get("legal_unit_id")) for row in rows if row.get("legal_unit_id")))
    units = {str(row.get("legal_unit_id")): row for row in store.legal_units}
    ancestors = {
        ancestor for unit_id in ids for ancestor in tuple(units.get(unit_id, {}).get("ancestor_legal_unit_ids") or ()) if ancestor in ids
    }
    return tuple(unit_id for unit_id in ids if unit_id not in ancestors)


def _prefer_parent_unit_ids(store: EvidenceStore, rows: tuple[dict, ...]) -> tuple[str, ...]:
    ids = tuple(dict.fromkeys(str(row.get("legal_unit_id")) for row in rows if row.get("legal_unit_id")))
    units = {str(row.get("legal_unit_id")): row for row in store.legal_units}
    descendants = {
        unit_id
        for unit_id in ids
        if any(ancestor in ids for ancestor in tuple(units.get(unit_id, {}).get("ancestor_legal_unit_ids") or ()))
    }
    return tuple(unit_id for unit_id in ids if unit_id not in descendants)


def _reference_for_requirement(row: dict) -> str:
    hierarchy = tuple(str(value).strip() for value in row.get("hierarchy") or () if str(value).strip())
    if len(hierarchy) >= 2 and re.fullmatch(r"\(\d+\)", hierarchy[-1]):
        return " ".join(hierarchy[-2:])
    return str(row.get("citation") or (hierarchy[-1] if hierarchy else "")).strip()


def _reference_parts(store: EvidenceStore, reference: str) -> tuple[str, ...]:
    """Delegate corpus-specific reference parsing to the registered adapter."""
    corpus_id = str(store.config.corpus_id)
    article = parse_pasal_reference(corpus_id, reference, allow_roman=True, config=store.config)
    paragraph = parse_ayat_reference(corpus_id, reference, config=store.config)
    return tuple(value for value in (article, paragraph) if value) or (reference,)
...
def _configured_requirements(
    ...
    if not isinstance(support_terms, dict):
        support_terms = {}
    authority_terms = string_tuple(support_terms.get("authority"))
    procedure_terms = string_tuple(support_terms.get("procedure"))
    if (
    ...
    if len(semantics.legal_references) > 1:
        reference_role = semantics.source_role or (semantics.source_scopes[0] if len(semantics.source_scopes) == 1 else None)
        return tuple(
            EvidenceRequirement(
                f"reference_{index}",
                description=reference,
    ...
    if _hinted(hints, normalized, "procedure") and (family := _procedure_family(research, normalized, entities)):
        return _procedure_requirements(store, query, research, family, entities, procedure_terms, minimums)
    if len(entities) > 1 and (_hinted(hints, normalized, "comparison") or _hinted(hints, normalized, "authority")):
        return tuple(
            EvidenceRequirement(
                f"entity_{index}",
                description=entity,
    ...
    if len(entities) > 1 and (_hinted(hints, normalized, "relation") or planner_requests_relation):
    ...
    if entities and _hinted(hints, normalized, "authority"):
        entity = entities[0]
        return (
            EvidenceRequirement(
                "authority",
...
def _instrument_comparison_requirements(
    ...
    operation_policy = research.get("operation_requirements", {})
    change_terms = string_tuple(operation_policy.get("change_relation_signals") if isinstance(operation_policy, dict) else ())
    relation_required = bool(semantics.requires_graph or semantics.relation_intent or contains_intent_phrase(query, change_terms))
        ...
        return tuple(
            EvidenceRequirement(
                ...
                retrieval_query=" ".join(
                    (
                        *string_tuple(stage.get("required_entities")),
                        *string_tuple(stage.get("support_terms")),
                    )
                ),
                required_entities=string_tuple(stage.get("required_entities")),
                support_terms=string_tuple(stage.get("support_terms")),
                authority_kinds=("normative_legal_text",),
...
def _hinted(hints: object, normalized: str, name: str) -> bool:
    values = hints.get(name, ()) if isinstance(hints, dict) else ()
...
def _procedure_family(research: dict, query: str, entities: tuple[str, ...]) -> str | None:
    ...
    for name, family in families.items():
        if not isinstance(family, dict):
            continue
        required_entities = string_tuple(family.get("required_entities"))
        signals = string_tuple(family.get("signals"))
...
def ambiguity_reason(semantics, routed: dict) -> str | None:
    ...
    entity_occurrences = (
        bool(routed.get("matches"))
        and all(row.get("fact_kind") == "person_role" and row.get("entity_identity") for row in routed.get("matches", ()))
