                ...
                rows.append(
                    {
                        ...
                        "status": status,
                        "text": pdf_page_text(doc, page_number, semantic_normalizer=semantic_normalizer),
                    }
                )
        finally:
            doc.close()
...
def pdf_page_text(doc, page_number: int, semantic_normalizer=None) -> str:
    text = doc[page_number - 1].get_text("text").replace("\xa0", " ").replace("\xad", "")
    normalizer = semantic_normalizer or normalize_semantic_text
    return "\n".join(normalizer(re.sub(r" {2,}", " ", line)) for line in text.split("\n"))
