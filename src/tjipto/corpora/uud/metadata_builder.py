...
from tjipto.corpora.uud.metadata_documents import (
    build_document_metadata as build_document_metadata,
    extract_decision_session,
    extract_metadata_date,
    ordinal_label,
    signatory_source_quotes,
)
        ...
        rows.append(
            {
                ...
                "supporting_evidence_id": metadata_grounding_id,
    ...
    def append_grounding(
        *,
        ...
        registry_rows = exact_bbox_rows or [
            ...
            for bbox_id, page_number in zip(bbox_refs, page_numbers, strict=True)
        ]
        ...
        field_rows.append(
            {
                ...
                "metadata_field": metadata_field,
                "metadata_grounding_id": field_id,
                "supporting_evidence_id": donor_id,
    ...
    for row in document_metadata:
        ...
        if role.startswith("amendment_"):
            ...
            if signatories:
                ...
                signatory_quotes = signatory_source_quotes(signatories["quoted_text"], row.get("signatories") or ())
            ...
            if decision:
                decision_date = extract_metadata_date(decision["quoted_text"])
                decision_session = extract_decision_session(decision["quoted_text"], decision_date)
                if decision_date:
