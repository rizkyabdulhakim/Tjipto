...
def _compare(case: dict[str, Any], response: dict, actual_ids: list[str], actual_citations: list[str]) -> list[str]:
    errors: list[str] = []
    ...
    if groups or alternatives:
        if not any(group <= set(actual_ids) for group in (*groups, *alternatives)):
