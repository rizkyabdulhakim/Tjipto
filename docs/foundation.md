...
Answer wording is composed from `AnswerDecision`, `ClaimSupport`, and the verified evidence projection. The separate cases in `tests/fixtures/uud/answer_cases.jsonl` evaluate factual units and publication contracts without prose snapshots. A configured shared provider is used for planning and verified-answer wording without feature flags; its owner is configured with `TJIPTO_LLM_PROVIDER`, `TJIPTO_LLM_MODEL`, `TJIPTO_LLM_API_KEY`, optional `TJIPTO_LLM_BASE_URL`, and optional `TJIPTO_LLM_TIMEOUT_SECONDS`; capability-specific `TJIPTO_RESEARCH_PLANNING_*` and `TJIPTO_WORDING_*` values override the shared setting. The planner may request one bounded clarification and the model may word only verified claims. Neither can publish facts, citations, viewer targets, authority, temporal context, finality, requirements, or sufficiency; invalid or unavailable output falls back to deterministic behavior.
