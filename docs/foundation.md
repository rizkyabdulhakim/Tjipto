...
Answer wording is composed deterministically from `AnswerDecision`, `ClaimSupport`, and the verified evidence projection. The separate cases in `tests/fixtures/uud/answer_cases.jsonl` evaluate factual units and publication contracts without prose snapshots. External Gemini wording is disabled by default and requires `TJIPTO_EXTERNAL_WORDING=gemini`; the provider receives only the deterministic answer and permitted facts, and its schema-constrained proposal may add neutral framing but cannot replace the deterministic facts or any server-owned support, citation, viewer, authority, temporal, or finality field. Missing or rejected provider output falls back to the deterministic answer.
