...
Answer wording is composed deterministically from `AnswerDecision`, `ClaimSupport`, and the verified evidence projection. The separate cases in `tests/fixtures/uud/answer_cases.jsonl` evaluate factual units and publication contracts without prose snapshots. Optional external wording requires `TJIPTO_EXTERNAL_WORDING=enabled` plus `TJIPTO_WORDING_PROVIDER`, `TJIPTO_WORDING_MODEL`, and `TJIPTO_WORDING_API_KEY`; an optional `TJIPTO_WORDING_BASE_URL` and `TJIPTO_WORDING_TIMEOUT_SECONDS` select a supported adapter. The model may choose only a server-rendered presentation primitive over the deterministic answer. It cannot publish prose, facts, citations, viewer targets, authority, temporal context, or finality; invalid or unavailable output falls back to the deterministic answer.
